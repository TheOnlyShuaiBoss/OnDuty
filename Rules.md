...
- **ZCode 桥接已可用（2026-09-21 完成，结论要点）**: 上述"待立项"已落地——`llm_proxy` 侧新增 `zcode_bridge.mjs`（常驻桥接，作宿主驱动官方 `zcode.cjs app-server`，模型请求由官方运行时发出、验证码在官方 renderer 内出码），DSH 里已能选到 `zcode/GLM-5.3-Flash`。**三条必须记住的结论**:
  ...
  ③ **运维前提**：ZCode 客户端须以**调试端口**运行（桥接出码必需，双击 `llm_proxy\test\zcode_probe\start_zcode_debug.bat`）；桥接随 `llm_proxy\start.bat` 拉起；`zcode` 平台已设 `skipDailyCheck`（否则每日自检会把模型逐个真打上游）。详见 llm_proxy `plans/2026-09-21-ZCode桥接常驻化方案.md` 与 `ISSUES.md`（Z1~Z5）。
