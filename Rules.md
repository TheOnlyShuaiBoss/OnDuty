...
- **ZCode 版本锁定（2026-09-20 用户拍板）**: 客户端固定 **3.11.2.6792，禁止升级**。实测原因: 3.12.3 起打包缺失 `resources\glm\provider\zcode-builtin.json`，CLI 启动即报"无法定位 CLI ZCode Built-in Provider Config"（0.8s 退出）；且升级会重建 `~\.zcode` 并清掉 `~\.zcode\cli\config.json`（随后报 Model config is missing）。若日后不得不再升级，按序恢复: ①跑 `scripts/sync-zcode-cli-config.ps1` 重建 CLI 配置（无 BOM）；②新版仍缺 provider 文件时，注入 `ZCODE_BUILTIN_PROVIDER_CONFIG_FILE` + `ZCODE_PERSONAL_PROVIDER_CONFIG_FILE` 绕过文件查找（3.12.3 实测有效）。
