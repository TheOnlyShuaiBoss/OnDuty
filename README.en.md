...
简体中文 README: [README.md](README.md) · Full manual: [docs/MANUAL.en.md](docs/MANUAL.en.md)（[中文](docs/MANUAL.md)）
...
|---|---|---|---|
| DSH (`dsh --profile headless`) | ✅ live-tested | ✗ upstream limit → use `{{prev.output}}` | none |
| CodeBuddy / WorkBuddy (client-bundled CLI or npm) | ✅ live-tested (`-p --output-format json`) | ✅ `--resume <id>` | one-time `/login` in the CLI |
| ZCode (client-bundled runtime or npm) | ✅ live-tested (`--prompt --json`) | ✅ `--resume sess_…` | one-time `login`; `scripts/sync-zcode-cli-config.ps1` reuses desktop provider |
| Any CLI with a non-interactive mode | ✅ | ✅ if it has one | `custom` adapter |
| claude code / codex / opencode | ✅ (docs-verified) | ✅ | planned v0.3 |
...
```
onduty/            # the daemon package (config/state/scheduler/runner/notify/adapters/cli)
docs/MANUAL.*.md   # full operations manual (zh/en)
plans/             # design docs: 000 master plan · 001/003 adapter live-tests · 002 naming
scripts/           # helpers (zcode CLI config sync, secret-free)
tasks.example.yaml # annotated reference config
tests/             # 59 unit tests, stdlib unittest
