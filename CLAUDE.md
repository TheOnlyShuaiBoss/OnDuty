...
- **适配纪律**: 内置 DSH / codebuddy(workbuddy 别名)/ zcode(占位降级)/ custom;各家 CLI 能力结论以 `plans/001_adapter_spike.md` 实测为准,禁止凭印象写 flag。
- **Windows 编码纪律**: agent 命令行一律 argv 直调,禁止 .cmd/.bat 包装中文/引号 prompt;落盘文件统一 UTF-8。
